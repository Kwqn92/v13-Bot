...
if (!role) return message.channel.send(' Lütfen Bir Rol Etiketleyin.').then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})
  ...
  return message.channel.send(` Verilecek rol sizin rolünüzün üstünde bu yüzden rolü **veremiyorum!**`).then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})
  }
  try{
await (member.roles.add(role.id))
 message.channel.send(new Discord.MessageEmbed().setDescription(` ${member} adlı kişiye \`${role.name}\` isimli Rol Başarıyla Verildi!`)  .setFooter('komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('#D2EE07')).then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})
