...
if(message.channel.id !== kanal) {
     const embed = new Discord.MessageEmbed()
  .setDescription(`<a:unlem:805037824369885214> Komutu kullanmak için <#${kanal}> kanalında Kullanılır`)
  .setColor('0x800d0d')
  .setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true }))
  .setTimestamp()
    message.channel.send({embeds: [embed]}).then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})
}
if (!bayan) return message.channel.send(`Sunucuda Kız Rolü Ayarlanmadığı İçin Komut Kullanılamaz ! `).then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})

 

let member = message.mentions.members.first();
if (!member) return message.channel.send(`Kız Olarak Kayıt Edeceğin Kullanıcıyı Belirtmelisin ! `).then(x => { message.delete();setTimeout(() => {x.delete()}, 5000)})
let isim = args[1]
...
member.roles.add(bayan)
      
  const embed = new Discord.MessageEmbed()
  .setDescription(`${member} **Adlı Kullanıcı Başarıyla Kız Olarak Kayıt Edildi!** Kayıt Eden Yetkili: ${message.member.displayName}\n\n Yeni İsim: ${isim} | ${yaş} \n Alınan Rol: <@&${kayıtsız}>\n Verilen Rol(ler): <@&${bayan}>`)
  .setColor('RANDOM')
  .setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true }))
  .setTimestamp()
  ...
  .setDescription(` ${member} **Adlı Kullanıcı Başarıyla Kız Olarak Kayıt Edildi.** `)
  .setColor('RANDOM')
  .setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true }))
  .setTimestamp()
